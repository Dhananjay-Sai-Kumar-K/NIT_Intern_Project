...
def load_dashboard_data() -> tuple[
    ...
    pd.DataFrame | None, gpd.GeoDataFrame | None
]:
    ...
    return cleaned_ais, segments, events, predictions, eez_gdf
...
def build_interactive_dashboard() -> None:
    cleaned_ais, segments, events, predictions, eez_gdf = load_dashboard_data()
    ...
    for rank, mmsi in enumerate(embed_mmsis):
        ...
        tracks_data.append({
            "mmsi":   mmsi,
            "rank":   rank,                   # 1-based index for the slider
            "color":  color,
            "points": n,
            "coords": coords,
    ...
    # ------------------------------------------------------------------
    # Inject vessel / event data + slider UI as raw HTML/JS
    # ------------------------------------------------------------------
...
function refreshMap(n) {{
    ...
    document.getElementById('event-count-label').textContent =
        eventMarkers.length + ' events shown';
}}
...
<!-- ---------------------------------------------------------------
     Slider control panel (fixed overlay)
--------------------------------------------------------------- -->
...
">
  ...
  <div style="font-size:11px;color:#6c757d;margin-bottom:6px;"
       id="event-count-label">0 events shown</div>
  ...
  <div style="font-size:10.5px;line-height:1.8;">
    ...
    <span style="color:{EVENT_COLORS['REPEATED_CROSSING']};font-weight:600;">&#9679;</span> REPEATED CROSSING
  </div>
  ...
  <div style="margin-top:8px;font-size:10px;color:#adb5bd;">
    ...
    Total events: {len(events_data_out):,}
  </div>
</div>
"""
    ...
    print(f"  Embedded {len(events_data_out)} events")
